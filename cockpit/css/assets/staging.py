...
@dbt_assets(
    ...
    select=CSS_SCOPE.dbt_selector,
    dagster_dbt_translator=PrefixTranslator("postgres", source_prefix=["css", "rate"]),
)
...
@dbt_assets(
    manifest=delta_dbt_project.manifest_path,
    select=CSS_SCOPE.dbt_selector,
    dagster_dbt_translator=PrefixTranslator("snowflake", source_prefix=["css", "rate"]),
)
