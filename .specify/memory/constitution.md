...
Changes SHOULD stay within the smallest domain slice that solves the problem. Backlog items and, when needed, specs MUST identify the minimum affected files, expected tests, and operational checks. Broad refactors MUST be separated into a distinct PR/Task from functional changes, and document the reasonaing in task plan.
